...
export const CodeGroupBottom = () => {
  const { activeSnippet } = useCodeBlockContext()
  ...
  return (
    <div className="border-t border-brand-w1/30 p-3 text-f-p-mobile text-brand-g1 lg:text-f-p flex items-center gap-2">
      ...
      <span className="text-brand-w1/50">•</span>
...
export const StandaloneCopyButton = ({ code }: { code: string }) => {
  ...
  return (
    <button
      className="absolute right-3 top-3 z-50 text-brand-w1 opacity-0 transition-all duration-200 group-hover:opacity-100"
      onClick={handleCopy}
    >
      <svg
        width="12"
